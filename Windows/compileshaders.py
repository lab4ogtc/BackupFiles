...
parser.add_argument('--target', type=str, help='Set the execution environment, same as --target-env for glslangvalidator')
...
for root, dirs, files in os.walk(dir_path):
    for file in files:
        if file.endswith(".vert") or file.endswith(".frag") or file.endswith(".comp") or file.endswith(".geom") or file.endswith(".tesc") or file.endswith(".tese") or file.endswith(".rgen") or file.endswith(".rchit") or file.endswith(".rmiss"):
            ...
            if args.target != None:
                add_params = add_params + " --target-env " + args.target

            if args.target == None and (file.endswith(".rgen") or file.endswith(".rchit") or file.endswith(".rmiss")):
               add_params = add_params + " --target-env vulkan1.2"

            print("%s -V %s -o %s %s" % (glslang_path, input_file, output_file, add_params))
